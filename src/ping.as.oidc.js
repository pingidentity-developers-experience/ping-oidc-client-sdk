...
const code_verifier = "Z7tkINgI-b2ftB_qgsFBHXSRK79gdTkNdTFeyk5fRZ-3bFrEWHzn.06XBg7Q-rd0yoFYrQ2c8nAkLpJ9cHc6QoI__eAwz0wQFkD_T3Jwr5xkqVYJwDzsdnT_zMcGa~Rk";
...
const oidc = () => {
    ...
    switch(responseType) {
        ...
        case 'client_credentials':
            ...
            break;
        case 'implicit':
            console.warn('ping-as-oidc.getToken', 'The implicit grant type is being deprecated by the IETF. Code grant type is recommended. See Oauth 2.1 specs.');
            // implementation
            break;
        case 'ropc':
            console.warn('ping-as-oidc.getToken', 'The ROPC grant type is being deprecated by the IETF. Code grant type is recommended. See Oauth 2.1 specs.');
            // implementation
            break;
    }
}
...
/**
   ...
   * @returns {string} An authorization code.
   */
const authorize = ({ clientId, responseType = "code", codeChallenge, codeChallengeMethod, state, fullRedirect = false } = {}) => {
  ...
  */
  ...
  const authzEndpoint = "/as/authorization.oauth2?client_id=ac_client&response_type=code&code_challenge=3miT2HK8Mo-KBfEyi4DAmo7uRYuqySPdUF1CEC91ty8&code_challenge_method=S256&state=california";
  if (fullRedirect) {
  ...
  switch (responseType) {
    case "code":
      ...
      let config = {
        ...
        headers: {
          ...
          Authorization: "Basic YWNfY2xpZW50OnAzRzk3bHRlWVpnRGRzWGh3UWo4TGQzOFdLT3JLalkyUnByQjFHTzg4bkFWalRzd0xhb0ExdFJhWkdCVXJBY1Y=",
        },
        timeout: 10,
        data: data,
      };
    ...
    case "client_credentials":
      ...
      break;
    case "implicit":
      console.warn("ping-as-oidc.authorize", "The implicit grant type is being deprecated by the IETF. Code grant type is recommended. See Oauth 2.1 specs."
      );
      // implementation
      break;
    case "ropc":
      console.warn("ping-as-oidc.authorize", "The ROPC grant type is being deprecated by the IETF. Code grant type is recommended. See Oauth 2.1 specs."
      );
      // implementation
      break;
  }
